...
TEST_F(TestFixture, MeshHeaders) {
   ...
   for (int meshid=1; meshid<=meshCount; ++meshid) {
      ...
      // generic mesh fields
      {
         ...
         EXPECT_EQ(0, avm_get_str(fileid, "description", description, descriptionSize));
         EXPECT_STREQ("A nonsensical unstruc mesh that just demonstrates the format.", description);
      }

      // unstruc mesh fields
      {
         EXPECT_EQ(0, avm_get_int(fileid, "fullViscousLayerCount", &i));
         EXPECT_EQ(0, i);
         EXPECT_EQ(0, avm_get_int(fileid, "partialViscousLayerCount", &i));
         EXPECT_EQ(0, i);
         ...
         EXPECT_EQ(0, avm_get_int(fileid, "nPatchNodes", &i));
         EXPECT_EQ(12, i);
         EXPECT_EQ(0, avm_get_int(fileid, "nPatchFaces", &i));
         EXPECT_EQ(14, i);
      }
