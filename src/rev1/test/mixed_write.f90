...
program mixed_write
 ...
 ! initialization
   ...
   call avm_write_mesh_header(10, mesh_hdrs(3))
   call avm_write_unstruc_header(10, unstruc_hdr, unstruc_patchhdrs)

   call avm_write_mesh_header(10, mesh_hdrs(4))
...
subroutine init_bfstruc_mesh
   ...
   mesh_hdrs(1)%meshDescription = 'A nonsensical bfstruc mesh that just demonstrates the format.'
...
subroutine init_cart_mesh
   ...
   mesh_hdrs(4)%referenceArea            = 1.0
   mesh_hdrs(4)%referencePoint(:)          = (/10.0,11.0,12.0/)
   mesh_hdrs(4)%referencePointDescription = 'origin'
   mesh_hdrs(1)%meshDescription = 'A nonsensical cart mesh that just demonstrates the format.'
