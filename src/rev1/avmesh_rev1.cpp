...
int rev1::avm_open(rev1_avmesh_file* avf, int* id)
{
#define CHECK_RETURN(v) if(v) { return 1; }
   ...
   if(!rev1::file_header_t::read(avf->fp, avf->byte_swap, &avf->file_hdr)) {
...
int rev1::avm_read_mesh_headers(rev1_avmesh_file* avf)
{
   ...
   for (int i=0; i<avf->file_hdr.meshCount; ++i)
   {
      ...
      if (0==strncmp("bfstruc", mesh_hdr.meshType, strlen("bfstruc"))) {
         if(!bfstruc_info::read(avf->fp, avf->byte_swap, &avf->bfstruc[i])) {
            RETURN_ERROR("avm_read_mesh_headers: failed reading bfstruc header");
         }
         ...
         if(!cart_info::read(avf->fp, avf->byte_swap, &avf->cart[i])) {
            RETURN_ERROR("avm_read_mesh_headers: failed reading cart header");
         }
...
int rev1::avm_new_file(rev1_avmesh_file* avf, const char* filename)
{
...
int rev1::avm_write_headers(rev1_avmesh_file* avf)
{
   ...
   for (int i=0; i<avf->file_hdr.meshCount; ++i)
   {
      ...
      if (0==strncmp("bfstruc", mesh_hdr.meshType, strlen("bfstruc"))) {
         if (!bfstruc_info::write(avf->fp, avf->byte_swap, &avf->bfstruc[i])) {
            RETURN_ERROR("avm_write_headers: failed writing bfstruc header");
         }
      } else if (0==strncmp("strand", mesh_hdr.meshType, strlen("strand"))) {
         ...
         if (!cart_info::write(avf->fp, avf->byte_swap, &avf->cart[i])) {
            RETURN_ERROR("avm_write_headers: failed writing cart header");
         }
...
int rev1::avm_select(rev1_avmesh_file* avf, const char* section, int id)
{
...
int rev1::avm_get_int(rev1_avmesh_file* avf, const char* field, int* value)
{
...
#define RETURN_ANSWER(a) {*value=a; return 0;}
   ...
   if (bfstruc_mesh) {
      if (FIELD_EQ("jmax")) RETURN_ANSWER(avf->bfstruc[meshid].header.jmax)
      if (FIELD_EQ("kmax")) RETURN_ANSWER(avf->bfstruc[meshid].header.kmax)
      if (FIELD_EQ("lmax")) RETURN_ANSWER(avf->bfstruc[meshid].header.lmax)
      if (FIELD_EQ("iblank")) RETURN_ANSWER(avf->bfstruc[meshid].header.iblank)
      if (FIELD_EQ("nPatches")) RETURN_ANSWER(avf->bfstruc[meshid].header.nPatches)
      if (FIELD_EQ("nPatchIntParams")) RETURN_ANSWER(avf->bfstruc[meshid].header.nPatchIntParams)
      if (FIELD_EQ("nPatchR8Params")) RETURN_ANSWER(avf->bfstruc[meshid].header.nPatchR8Params)
      if (FIELD_EQ("nPatchC80Params")) RETURN_ANSWER(avf->bfstruc[meshid].header.nPatchC80Params)

      //all remaining bfstruc fields require a valid patchid
      if (patchid < 0 || patchid >= avf->bfstruc[meshid].header.nPatches) {
         RETURN_ERROR("avm_get_int: invalid patchid");
      }
      if (FIELD_EQ("bctype")) RETURN_ANSWER(avf->bfstruc[meshid].patches[patchid].bctype)
      if (FIELD_EQ("direction")) RETURN_ANSWER(avf->bfstruc[meshid].patches[patchid].direction)
      if (FIELD_EQ("jbegin")) RETURN_ANSWER(avf->bfstruc[meshid].patches[patchid].jbegin)
      if (FIELD_EQ("jend")) RETURN_ANSWER(avf->bfstruc[meshid].patches[patchid].jend)
      if (FIELD_EQ("kbegin")) RETURN_ANSWER(avf->bfstruc[meshid].patches[patchid].kbegin)
      if (FIELD_EQ("kend")) RETURN_ANSWER(avf->bfstruc[meshid].patches[patchid].kend)
      if (FIELD_EQ("lbegin")) RETURN_ANSWER(avf->bfstruc[meshid].patches[patchid].lbegin)
      if (FIELD_EQ("lend")) RETURN_ANSWER(avf->bfstruc[meshid].patches[patchid].lend)

      //all remaining bfstruc fields require a valid patchid and patchparam
      if (patchparam < 0 || patchparam >= avf->bfstruc[meshid].header.nPatchIntParams) {
         RETURN_ERROR("avm_get_int: invalid patchparam");
      }
      if (FIELD_EQ("patch_int_param"))
         RETURN_ANSWER(avf->bfstruc[meshid].patchIntParams[patchid][patchparam])
      RETURN_ERROR("avm_get_int: invalid field");
   }
   ...
   if (cart_mesh) {
      if (FIELD_EQ("nLevels")) RETURN_ANSWER(avf->cart[meshid].header.nLevels)
      if (FIELD_EQ("nBlocks")) RETURN_ANSWER(avf->cart[meshid].header.nBlocks)
      if (FIELD_EQ("nFringe")) RETURN_ANSWER(avf->cart[meshid].header.nFringe)
      if (FIELD_EQ("nxc")) RETURN_ANSWER(avf->cart[meshid].header.nxc)
      if (FIELD_EQ("nyc")) RETURN_ANSWER(avf->cart[meshid].header.nyc)
      if (FIELD_EQ("nzc")) RETURN_ANSWER(avf->cart[meshid].header.nzc)

      if (FIELD_EQ("iRatio")) {
         //this field requires a valid levelid
         if (levelid < 0 || levelid >= avf->cart[meshid].header.nLevels) {
            RETURN_ERROR("avm_get_int: invalid levelid");
         }
         RETURN_ANSWER(avf->cart[meshid].ratios[levelid])
      }
      //all remaining cart fields require a valid blockid
      if (blockid < 0 || blockid >= avf->cart[meshid].header.nBlocks) {
         RETURN_ERROR("avm_get_int: invalid blockid");
      }
      if (FIELD_EQ("levNum")) RETURN_ANSWER(avf->cart[meshid].blocks[blockid].levNum)
      if (FIELD_EQ("iblFlag")) RETURN_ANSWER(avf->cart[meshid].blocks[blockid].iblFlag)
      if (FIELD_EQ("bdryFlag")) RETURN_ANSWER(avf->cart[meshid].blocks[blockid].bdryFlag)
      RETURN_ERROR("avm_get_int: invalid field");
   }
...
int rev1::avm_get_real(rev1_avmesh_file* avf, const char* field, double* value)
{
#define FIELD_EQ(s) 0==strcmp(s,field)
#define RETURN_ANSWER(a) {*value=a; return 0;}
   ...
   if (bfstruc_mesh) {
      if (FIELD_EQ("patch_r8_param")) {
         //this field requires a valid patchid and patchparam
         if (patchid < 0 || patchid >= avf->bfstruc[meshid].header.nPatches) {
            RETURN_ERROR("avm_get_real: invalid patchid");
         }
         if (patchparam < 0 || patchparam >= avf->bfstruc[meshid].header.nPatchR8Params) {
               RETURN_ERROR("avm_get_real: invalid patchparam");
         }
         RETURN_ANSWER(avf->bfstruc[meshid].patchR8Params[patchid][patchparam])
      }
   ...
   if (cart_mesh) {
      if (FIELD_EQ("domXLo")) RETURN_ANSWER(avf->cart[meshid].header.domXLo)
      if (FIELD_EQ("domYLo")) RETURN_ANSWER(avf->cart[meshid].header.domYLo)
      if (FIELD_EQ("domZLo")) RETURN_ANSWER(avf->cart[meshid].header.domZLo)
      if (FIELD_EQ("domXHi")) RETURN_ANSWER(avf->cart[meshid].header.domXHi)
      if (FIELD_EQ("domYHi")) RETURN_ANSWER(avf->cart[meshid].header.domYHi)
      if (FIELD_EQ("domZHi")) RETURN_ANSWER(avf->cart[meshid].header.domZHi)
      RETURN_ERROR("avm_get_real: invalid field");  
   }
...
int rev1::avm_get_str(rev1_avmesh_file* avf, const char* field, char* str, int len)
{
...
#define RETURN_ANSWER(a,l) {strncpy(str,a,l); for (int i=len-1; i>=0 && (isspace(str[i]) || str[i]==0); --i) str[i]=0; str[len-1]=0; return 0;}
   ...
   if (FIELD_EQ("AVM_WRITE_ENDIAN")) {
      ...
      if (avf->write_endian==rev1_avmesh_file::ENDIAN_LITTLE) endian = "little";
      if (avf->write_endian==rev1_avmesh_file::ENDIAN_BIG) endian = "big";
   ...
   if (0 <= meshid && meshid < avf->file_hdr.meshCount) { // mesh_header 
      ...
      if (bfstruc_mesh) {
         ...
         if (FIELD_EQ("patch_description")) RETURN_ANSWER(avf->bfstruc[meshid].patches[patchid].description, JMIN(len,AVM_STD_STRING_LENGTH))

         //all remaining bfstruc fields require a valid patchid and patchparam
         if (patchparam < 0 || patchparam >= avf->bfstruc[meshid].header.nPatchC80Params) {
            RETURN_ERROR("avm_get_str: invalid patchparam");
         }
         if (FIELD_EQ("patch_c80_param")) {
            string tmp = avf->bfstruc[meshid].patchC80Params[patchid][patchparam];
            RETURN_ANSWER(tmp.c_str(), JMIN(len,80))
         }
...
int rev1::avm_get_int_array(rev1_avmesh_file* avf, const char* field, int* values, int len)
{
#define FIELD_EQ(s) 0==strcmp(s,field)
#define RETURN_ANSWER(a,l) {for(int i=0;i<l;++i) values[i]=a[i]; return 0;}
   ...
   if (cart_mesh) {
      //all remaining cart fields require a valid meshid and blockid
      if (meshid < 0 || meshid >= avf->file_hdr.meshCount) {
         RETURN_ERROR("avm_get_int_array: invalid meshid");
      }
      if (blockid < 0 || blockid >= avf->cart[meshid].header.nBlocks) {
         RETURN_ERROR("avm_get_int_array: invalid blockid");
      }
      if (FIELD_EQ("ilo")) RETURN_ANSWER(avf->cart[meshid].blocks[blockid].ilo, min(len,3))
      if (FIELD_EQ("ihi")) RETURN_ANSWER(avf->cart[meshid].blocks[blockid].ihi, min(len,3))
      if (FIELD_EQ("iblDim")) RETURN_ANSWER(avf->cart[meshid].blocks[blockid].iblDim, min(len,3))
      if (FIELD_EQ("blo")) RETURN_ANSWER(avf->cart[meshid].blocks[blockid].blo, min(len,3))
      if (FIELD_EQ("bhi")) RETURN_ANSWER(avf->cart[meshid].blocks[blockid].bhi, min(len,3))
   }
...
int rev1::avm_get_real_array(rev1_avmesh_file* avf, const char* field, double* values, int len)
{
#define FIELD_EQ(s) 0==strcmp(s,field)
#define RETURN_ANSWER(a,l) {for(int i=0;i<l;++i) values[i]=a[i]; return 0;}
...
int rev1::avm_set_int(rev1_avmesh_file* avf, const char* field, int value)
{
#define FIELD_EQ(s) 0==strcmp(s,field)
#define SET_AND_RETURN(a) {a=value; return 0;}
   ...
   if (bfstruc_mesh) {
      ...
      //all remaining bfstruc fields require a valid patchid
      if (patchid < 0 || patchid >= avf->bfstruc[meshid].header.nPatches) {
         RETURN_ERROR("avm_set_int: invalid patchid");
      }
      ...
      //all remaining bfstruc fields require a valid patchid and patchparam
      if (patchparam < 0 || patchparam >= avf->bfstruc[meshid].header.nPatchIntParams) {
         RETURN_ERROR("avm_set_int: invalid patchparam");
      }
      if (FIELD_EQ("patch_int_param"))
   ...
   if (cart_mesh) {
      ...
      if (FIELD_EQ("iRatio")) {
         //this field requires a valid edgepatch
         if (levelid < 0 || levelid >= avf->cart[meshid].header.nLevels) {
            RETURN_ERROR("avm_set_int: invalid levelid");
         }
      ...
      //all remaining cart fields require a valid blockid
      if (blockid < 0 || blockid >= avf->cart[meshid].header.nBlocks) {
         RETURN_ERROR("avm_set_int: invalid blockid");
      }
...
int rev1::avm_set_real(rev1_avmesh_file* avf, const char* field, double value)
{
#define FIELD_EQ(s) 0==strcmp(s,field)
#define SET_AND_RETURN(a) {a=value; return 0;}
   ...
   if (bfstruc_mesh && FIELD_EQ("patch_r8_param")) {
      //all remaining bfstruc fields require a valid patchid
      if (patchid < 0 || patchid >= avf->bfstruc[meshid].header.nPatches) {
         RETURN_ERROR("avm_set_real: invalid patchid");
      }
      //all remaining bfstruc fields require a valid patchparam
      if (patchparam < 0 || patchparam >= avf->bfstruc[meshid].header.nPatchR8Params) {
         RETURN_ERROR("avm_set_real: invalid patchparam");
      }
...
int rev1::avm_set_str(rev1_avmesh_file* avf, const char* field, const char* str, int len)
{
...
#define SETSTR_AND_RETURN(a,l) {a=string(str,str+l); return 0;}
   ...
   if (FIELD_EQ("AVM_WRITE_ENDIAN")) {
      if (0==strcmp(str,"big")) avf->write_endian = rev1_avmesh_file::ENDIAN_BIG;
   ...
   if (bfstruc_mesh) {
      ...
      if (FIELD_EQ("patch_c80_param")) {
         //this field requires a valid patchparam
         if (patchparam < 0 || patchparam >= avf->bfstruc[meshid].header.nPatchC80Params) {
...
int rev1::avm_set_int_array(rev1_avmesh_file* avf, const char* field, const int* values, int len)
{
#define FIELD_EQ(s) 0==strcmp(s,field)
#define SET_AND_RETURN(a,l) {for(int i=0;i<l;++i) a[i]=values[i]; return 0;}
   ...
   if (cart_mesh) {
      if (meshid < 0 || meshid >= avf->file_hdr.meshCount) {
         RETURN_ERROR("avm_set_int_array: invalid meshid");
      }
      //all remaining cart fields require a valid blockid
      if (blockid < 0 || blockid >= avf->cart[meshid].header.nBlocks) {
         RETURN_ERROR("avm_set_int_array: invalid blockid");
      }
      ...
      RETURN_ERROR("avm_set_int_array: invalid field");
   }
...
int rev1::avm_set_real_array(rev1_avmesh_file* avf, const char* field, const double* values, int len)
{
#define FIELD_EQ(s) 0==strcmp(s,field)
#define SET_AND_RETURN(a,l) {for(int i=0;i<l;++i) a[i]=values[i]; return 0;}
...
int rev1::avm_mesh_header_offset(rev1_avmesh_file* avf, int mesh, off_t* offset)
{
...
int rev1::avm_mesh_data_offset(rev1_avmesh_file* avf, int mesh, off_t* offset)
{
...
int rev1::seek(FILE* fp, off_t offset)
{
#ifdef WIN32
...
int rev1::avm_seek_to_mesh(rev1_avmesh_file* avf, int mesh)
{
...
int rev1::avm_bfstruc_read_xyz_r4(
   rev1_avmesh_file* avf,
   float* x,
   float* y,
   float* z,
   int size
)
{
   ...
   if (!avf) RETURN_ERROR("avm_bfstruc_read_xyz_r4: avf is NULL");

   int meshid = avf->selection_meshid-1; 
   if (meshid < 0 || meshid >= avf->file_hdr.meshCount) {
      RETURN_ERROR("avm_bfstruc_read_xyz_r4: invalid meshid");
   }
   ...
   if (array_size != size) RETURN_ERROR("avm_bfstruc_read_xyz_r4: size does not match header");

   if (!fread(x, sizeof(float)*array_size, 1, avf->fp)) RETURN_ERROR("avm_bfstruc_read_xyz_r4: failed reading x array");
   if (!fread(y, sizeof(float)*array_size, 1, avf->fp)) RETURN_ERROR("avm_bfstruc_read_xyz_r4: failed reading y array");
   if (!fread(z, sizeof(float)*array_size, 1, avf->fp)) RETURN_ERROR("avm_bfstruc_read_xyz_r4: failed reading z array");
...
int rev1::avm_bfstruc_read_xyz_r8(
   rev1_avmesh_file* avf,
   double* x,
   double* y,
   double* z,
...
)
{
   ...
   if (!avf) RETURN_ERROR("avm_bfstruc_read_xyz_r8: avf is NULL");

   int meshid = avf->selection_meshid-1; 
   if (meshid < 0 || meshid >= avf->file_hdr.meshCount) {
      RETURN_ERROR("avm_bfstruc_read_xyz_r8: invalid meshid");
   }
   ...
   int array_size =
      (avf->bfstruc[meshid].header.jmax *
       avf->bfstruc[meshid].header.kmax *
       avf->bfstruc[meshid].header.lmax);
   if (array_size != size) RETURN_ERROR("avm_bfstruc_read_xyz_r8: size does not match header");

   if (!fread(x, sizeof(double)*array_size, 1, avf->fp)) RETURN_ERROR("avm_bfstruc_read_xyz_r8: failed reading x array");
   if (!fread(y, sizeof(double)*array_size, 1, avf->fp)) RETURN_ERROR("avm_bfstruc_read_xyz_r8: failed reading y array");
   if (!fread(z, sizeof(double)*array_size, 1, avf->fp)) RETURN_ERROR("avm_bfstruc_read_xyz_r8: failed reading z array");
...
int rev1::avm_bfstruc_read_iblank(
   rev1_avmesh_file* avf,
   int* iblanks,
   int size
)
{
   ...
   if (!avf) RETURN_ERROR("avm_bfstruc_read_iblank: avf is NULL");

   int meshid = avf->selection_meshid-1; 
   if (meshid < 0 || meshid >= avf->file_hdr.meshCount) {
      RETURN_ERROR("avm_bfstruc_read_iblank: invalid meshid");
   }
   ...
   int array_size =
      (avf->bfstruc[meshid].header.jmax *
       avf->bfstruc[meshid].header.kmax *
       avf->bfstruc[meshid].header.lmax);
   if (array_size != size) RETURN_ERROR("avm_bfstruc_read_iblank: size does not match header");

   if (!fread(iblanks, sizeof(int)*array_size, 1, avf->fp)) RETURN_ERROR("avm_bfstruc_read_iblank: failed reading iblanks array");
...
int rev1::avm_bfstruc_write_xyz_r4(rev1_avmesh_file* avf, float* x, float* y, float* z, int size)
{
   ...
   if (!avf) RETURN_ERROR("avm_bfstruc_write_xyz_r4: avf is NULL");

   int meshid = avf->selection_meshid-1; 
   if (meshid < 0 || meshid >= avf->file_hdr.meshCount) {
      RETURN_ERROR("avm_bfstruc_write_xyz_r4: invalid meshid");
   }
   ...
   int array_size =
      (avf->bfstruc[meshid].header.jmax *
       avf->bfstruc[meshid].header.kmax *
       avf->bfstruc[meshid].header.lmax);
   if (array_size != size) RETURN_ERROR("avm_bfstruc_write_xyz_r4: size does not match header");
   ...
   if (!fwrite(x, sizeof(float)*array_size, 1, avf->fp)) RETURN_ERROR("avm_bfstruc_write_xyz_r4: failed writing x array");
   if (!fwrite(y, sizeof(float)*array_size, 1, avf->fp)) RETURN_ERROR("avm_bfstruc_write_xyz_r4: failed writing y array");
   if (!fwrite(z, sizeof(float)*array_size, 1, avf->fp)) RETURN_ERROR("avm_bfstruc_write_xyz_r4: failed writing z array");
...
int rev1::avm_bfstruc_write_xyz_r8(rev1_avmesh_file* avf, double* x, double* y, double* z, int size)
{
   ...
   if (!avf) RETURN_ERROR("avm_bfstruc_write_xyz_r8: avf is NULL");

   int meshid = avf->selection_meshid-1; 
   if (meshid < 0 || meshid >= avf->file_hdr.meshCount) {
      RETURN_ERROR("avm_bfstruc_write_xyz_r8: invalid meshid");
   }
   ...
   int array_size =
      (avf->bfstruc[meshid].header.jmax *
       avf->bfstruc[meshid].header.kmax *
       avf->bfstruc[meshid].header.lmax);
   if (array_size != size) RETURN_ERROR("avm_bfstruc_write_xyz_r8: size does not match header");
   ...
   if (!fwrite(x, sizeof(double)*array_size, 1, avf->fp)) RETURN_ERROR("avm_bfstruc_write_xyz_r8: failed writing x array");
   if (!fwrite(y, sizeof(double)*array_size, 1, avf->fp)) RETURN_ERROR("avm_bfstruc_write_xyz_r8: failed writing y array");
   if (!fwrite(z, sizeof(double)*array_size, 1, avf->fp)) RETURN_ERROR("avm_bfstruc_write_xyz_r8: failed writing z array");
...
int rev1::avm_bfstruc_write_iblank(rev1_avmesh_file* avf, int* iblanks, int size)
{
   if (iblanks==NULL) RETURN_ERROR("avm_bfstruc_write_iblank: iblanks is NULL");
   if (!avf) RETURN_ERROR("avm_bfstruc_write_iblank: avf is NULL");

   int meshid = avf->selection_meshid-1; 
   if (meshid < 0 || meshid >= avf->file_hdr.meshCount) {
      RETURN_ERROR("avm_bfstruc_write_iblank: invalid meshid");
   }
   ...
   int array_size =
      (avf->bfstruc[meshid].header.jmax *
       avf->bfstruc[meshid].header.kmax *
       avf->bfstruc[meshid].header.lmax);
   if (array_size != size) RETURN_ERROR("avm_bfstruc_write_iblank: size does not match header");

   if (avf->byte_swap)
      for (int i=0; i<array_size; ++i)
         byte_swap_int(iblanks+i);

   if (!fwrite(iblanks, sizeof(int)*array_size, 1, avf->fp)) RETURN_ERROR("avm_bfstruc_write_iblank: failed writing iblanks array");
...
int rev1::avm_strand_read_nodes_r4(rev1_avmesh_file* avf,
...
int rev1::avm_strand_read_nodes_r8(rev1_avmesh_file* avf,
...
int rev1::avm_strand_read_faces(rev1_avmesh_file* avf,
...
int rev1::avm_strand_read_edges(rev1_avmesh_file* avf,
...
int rev1::avm_strand_read_amr(rev1_avmesh_file* avf,
...
int rev1::avm_strand_write_faces(rev1_avmesh_file* avf,
   int* triFaces,       int triFaces_size,
   int* quadFaces,      int quadFaces_size,
   int* polyFaces,      int polyFaces_size,
   int* faceTag,       int faceTag_size
...
int rev1::avm_strand_write_nodes_r4(rev1_avmesh_file* avf,
   float* xyz,         int xyz_size,
   int* nodeID,        int nodeID_size,
   int* nodeClip,      int nodeClip_size,
   float* pointingVec, int pointingVec_size,
...
int rev1::avm_strand_write_nodes_r8(rev1_avmesh_file* avf,
   double* xyz,         int xyz_size,
   int* nodeID,        int nodeID_size,
   int* nodeClip,      int nodeClip_size,
   double* pointingVec, int pointingVec_size,
...
int rev1::avm_strand_write_edges(rev1_avmesh_file* avf,
   int* bndEdges,      int bndEdges_size,
   int* edgeTag,       int edgeTag_size
)
{
...
int rev1::avm_strand_write_amr(rev1_avmesh_file* avf,
   AMR_Node_Data* nodesOnGeometry, int nodesOnGeometry_size,
   int* edgesOnGeometry, int edgesOnGeometry_size,
   int* facesOnGeometry, int facesOnGeometry_size
)
...
int rev1::avm_unstruc_read_nodes_r4(rev1_avmesh_file* avf, float* xyz, int xyz_size)
{
...
int rev1::avm_unstruc_read_nodes_r8(rev1_avmesh_file* avf, double* xyz, int xyz_size)
{
...
int rev1::avm_unstruc_read_partial_nodes_r4(rev1_avmesh_file* avf, float* xyz, int xyz_size, int start, int end)
{
...
int rev1::avm_unstruc_read_partial_nodes_r8(rev1_avmesh_file* avf, double* xyz, int xyz_size, int start, int end)
{
...
int rev1::avm_unstruc_read_faces(rev1_avmesh_file* avf,
   int* triFaces,  int triFaces_size,
   int* quadFaces, int quadFaces_size,
   int* polyFaces, int polyFaces_size
)
...
int rev1::avm_unstruc_read_tri(rev1_avmesh_file* avf,
   int* triFaces, int triFaces_size,
...
int rev1::avm_unstruc_read_quad(rev1_avmesh_file* avf,
   int* quadFaces, int quadFaces_size,
   int stride, int start, int end, int flags
)
{
...
int rev1::avm_unstruc_read_partial_faces(rev1_avmesh_file* avf,
...
int rev1::avm_unstruc_read_bnd_faces(rev1_avmesh_file* avf,
...
int rev1::avm_unstruc_read_cells(rev1_avmesh_file* avf,
   ...
   int* pyrCells, int pyrCells_size)
{
   ...
   return(rev1::avm_unstruc_read_partial_cells(avf,
...
int rev1::avm_unstruc_read_partial_cells(rev1_avmesh_file* avf,
...
int rev1::avm_unstruc_read_edges(rev1_avmesh_file* avf, int* edges, int edges_size)
{
...
int rev1::avm_unstruc_read_amr(rev1_avmesh_file* avf,
   AMR_Node_Data* nodesOnGeometry, int nodesOnGeometry_size,
   int* edgesOnGeometry, int edgesOnGeometry_size,
   int* facesOnGeometry, int facesOnGeometry_size)
{
...
int rev1::avm_unstruc_read_amr_volumeids(rev1_avmesh_file* avf,
...
int rev1::avm_unstruc_write_nodes_r4(rev1_avmesh_file* avf, float* xyz, int xyz_size)
{
...
int rev1::avm_unstruc_write_nodes_r8(rev1_avmesh_file* avf, double* xyz, int xyz_size)
{
...
int rev1::avm_unstruc_write_bnd_faces(rev1_avmesh_file* avf,
   int* triFaces,  int triFaces_size,
   int* quadFaces, int quadFaces_size,
   int* polyFaces, int polyFaces_size
)
...
int rev1::avm_unstruc_write_faces(rev1_avmesh_file* avf,
   int* triFaces,  int triFaces_size,
   int* quadFaces, int quadFaces_size,
   int* polyFaces, int polyFaces_size
)
...
int rev1::avm_unstruc_write_cells(rev1_avmesh_file* avf,
   int* hexCells, int hexCells_size,
   int* tetCells, int tetCells_size,
   int* priCells, int priCells_size,
   int* pyrCells, int pyrCells_size)
...
int rev1::avm_unstruc_write_edges(rev1_avmesh_file* avf, int* edges, int edges_size)
{
...
int rev1::avm_unstruc_write_amr(rev1_avmesh_file* avf,
   AMR_Node_Data* nodesOnGeometry, int nodesOnGeometry_size,
   int* edgesOnGeometry, int edgesOnGeometry_size,
   int* facesOnGeometry, int facesOnGeometry_size)
{
...
int rev1::avm_unstruc_write_amr_volumeids(rev1_avmesh_file* avf,
   int* hexGeomIds, int hexGeomIds_size,
   int* tetGeomIds, int tetGeomIds_size,
   int* priGeomIds, int priGeomIds_size,
   int* pyrGeomIds, int pyrGeomIds_size)
...
int rev1::avm_cart_read_iblank(rev1_avmesh_file* avf, int* iblanks, int size)
{
   if (iblanks==NULL) RETURN_ERROR("avm_cart_read_iblank: iblanks is NULL");
   if (!avf) RETURN_ERROR("avm_cart_read_iblank: avf is NULL");

   int meshid = avf->selection_meshid-1; 
   int blockid = avf->selection_blockid-1; 
   if (meshid < 0 || meshid >= avf->file_hdr.meshCount) {
      RETURN_ERROR("avm_cart_read_iblank: invalid meshid");
   }
   if (0!=strncmp("cart", avf->mesh_hdrs[meshid].meshType, strlen("cart"))) {
      RETURN_ERROR("avm_card_read_iblank: selected mesh is not a cart mesh");
   }
   if (blockid < 0 || blockid >= avf->cart[meshid].header.nBlocks) {
      RETURN_ERROR("avm_cart_read_iblank: invalid blockid");
   }
   int* d = avf->cart[meshid].blocks[blockid].iblDim;
   if (d[0]*d[1]*d[2] != size) {
      RETURN_ERROR("avm_cart_read_iblank: size does not match header");
   }

   if (!fread(iblanks, sizeof(int)*size, 1, avf->fp)) {
      RETURN_ERROR("avm_cart_read_iblank: failed reading iblanks array");
   }
...
int rev1::avm_cart_write_iblank(rev1_avmesh_file* avf, int* iblanks, int size)
{
   if (iblanks==NULL) RETURN_ERROR("avm_cart_write_iblank: iblanks is NULL");
   if (!avf) RETURN_ERROR("avm_cart_write_iblank: avf is NULL");

   int meshid = avf->selection_meshid-1; 
   int blockid = avf->selection_blockid-1; 
   if (!(0 <= meshid && meshid < avf->file_hdr.meshCount)) return 1; 
   if (meshid < 0 || meshid >= avf->file_hdr.meshCount) {
      RETURN_ERROR("avm_cart_write_iblank: invalid meshid");
   }
   if (0!=strncmp("cart", avf->mesh_hdrs[meshid].meshType, strlen("cart"))) {
      RETURN_ERROR("avm_card_write_iblank: selected mesh is not a cart mesh");
   }
   if (blockid < 0 || blockid >= avf->cart[meshid].header.nBlocks) {
      RETURN_ERROR("avm_cart_write_iblank: invalid blockid");
   }
   int* d = avf->cart[meshid].blocks[blockid].iblDim;
   if (d[0]*d[1]*d[2] != size) {
      RETURN_ERROR("avm_cart_write_iblank: size does not match header");
   }
   ...
   if (!fwrite(iblanks, sizeof(int)*size, 1, avf->fp)) {
      RETURN_ERROR("avm_cart_read_iblank: failed writing iblanks array");
   }
